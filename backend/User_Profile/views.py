...
class GoogleLogin(APIView):  # Inherit from APIView
...
class GoogleLoginCallback(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "error" in token_json:
            return Response(
                {"error": token_json.get("error_description", "Unknown error")},
                status=400,
            )
        # Verify the ID token
        try:
        ...
        # send request to home page        # Return your own JWT tokens to the frontend
        return Response(
            {
